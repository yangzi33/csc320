...
def calculate_homography(source, destination):
    ...
    ####### PUT YOUR CODE HERE #####
    ################################
    ####### TODO. Do more tests since it still renders empty image ############
    # source = H dest
    # p source
    # p' destination
    ...
    x1, x2, x3, x4 = destination[0, 0], destination[1, 0], destination[2, 0], destination[3, 0]
    y1, y2, y3, y4 = destination[0, 1], destination[1, 1], destination[2, 1], destination[3, 1]
    ...
    homography = np.linalg.solve(A, b).reshape(3, 3)
    #################################
    ######### DO NOT MODIFY #########
    #################################
    return homography
...
def backward_mapping(transform, source_image, destination_image, destination_coords):
    ...
    for r in range(h):
        ...
        for c in range(w):
            ...
            c_prime, r_prime = img_ops.unnormalize_coordinates(euclidean_coord, h, w)
            r_prime, c_prime = int(r_prime), int(c_prime)
            output_buffer[r, c] = source_image[r_prime, c_prime] 
    #################################
    ######### DO NOT MODIFY #########
    #################################
    return output_buffer
